...
export function useEditorialManager(
  ...
  const [step, setStep] = useState<"upload" | "availability" | "preview" | "difficulty">(
    "upload",
  );
  ...
  >([]);
  const [selectedDifficultSubjects, setSelectedDifficultSubjects] = useState<
    Set<string>
  >(new Set());
  ...
  const handleGenerateSchedule = async () => {
    ...
    try {
      ...
      if (data.schedule) {
        ...
        // Extract unique subjects from priorities (using subjectId for filtering)
        const subjects = Array.from(
          new Map(
            (data.schedule.priorities || [])
              .filter((p) => p.subjectId) // Only include if subjectId exists
              .map((p) => [
                p.subjectId,
                { id: p.subjectId!, name: p.subjectName || "Unknown" },
              ])
          ).values()
        );
        setExtractedSubjects(subjects);
        setSelectedDifficultSubjects(new Set());
        setStep(subjects.length > 0 ? "difficulty" : "preview");
  ...
  const handleImportSchedule = async () => {
    ...
    try {
      ...
      // Save difficult subjects preference
      if (selectedDifficultSubjects.size > 0) {
        try {
          await fetch("/api/users/preferences", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              difficultSubjects: Array.from(selectedDifficultSubjects),
            }),
          });
        } catch {
          // Silently fail, don't block import
        }
      }
  ...
  const handleSelectDifficulty = (subjectId: string) => {
    setSelectedDifficultSubjects((prev) => {
  ...
  const handleContinueFromDifficulty = () => {
    setStep("preview");
  };
