...
export function EditorialList({ editorials, onSelectEdito }: EditorialListProps) {
  ...
  if (editorials.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 px-4">
        ...
        <h3 className="text-lg font-semibold text-zinc-200 mb-2">Nenhum edital adicionado</h3>
  ...
  return (
    <div className="space-y-4">
      {editorials.map((editorial) => (
        <Card key={editorial.id} className="p-4 bg-card/50 border-white/10 hover:border-white/20 transition-colors">
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1 cursor-pointer" onClick={() => onSelectEdito?.(editorial)}>
              <div className="flex items-center gap-3 mb-2">
                <h3 className="font-semibold text-white text-lg">{editorial.title}</h3>
                {editorial.url && (
              ...
              {editorial.description && (
                <p className="text-sm text-zinc-400 mb-2">{editorial.description}</p>
              )}
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-zinc-500">
                  Adicionado em {format(new Date(editorial.uploadedAt), 'dd MMM yyyy', { locale: ptBR })}
                </span>
              </div>
            </div>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Esta ação não pode ser desfeita. Todos os mapeamentos de conteúdo também serão removidos.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
