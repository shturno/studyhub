...
import { Plus, Book, UploadCloud, Cpu, ArrowRight, Calendar, AlertCircle } from 'lucide-react'
...
export function EditorialManager({ contestId, role, examDate: examDateProp, onEditorialAdded }: Readonly<EditorialManagerProps>) {
  ...
  const [examDate, setExamDate] = useState(examDateProp ? examDateProp.split('T')[0] : '')
  const [generatedSchedule, setGeneratedSchedule] = useState<(GeneratedSchedule & { priorities?: StudyAreaPriority[] }) | null>(null)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data.schedule && data.schedule.weeks > 0) {
        toast.success('SUCESSO CRÍTICO: Edital, Disciplinas e Tópicos extraídos! Gerando cronograma...')
        ...
        if (data.usedDefaultExamDate) {
          toast.info('Data da prova não informada. Cronograma gerado com base em 6 meses.', { duration: 6000 })
        }
      } else {
  ...
  const handleImportSchedule = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'Erro ao importar cronograma')
    } finally {
        ...
        <DialogContent className="w-[95vw] sm:max-w-[500px] max-h-[90vh] overflow-y-auto border-0 rounded-none p-0" style={{ background: '#0a0005', border: '2px solid #ff006e', boxShadow: '0 0 30px rgba(255,0,110,0.2)' }}>
          <div className="p-4 sticky top-0 z-10 bg-[#0a0005]" style={{ borderBottom: '2px dashed #ff006e' }}>
            <DialogHeader>
              <DialogTitle className="font-pixel text-lg text-[#ff006e] flex items-center gap-2">
                ...
                {generatedSchedule ? 'PREVIEW DO CRONOGRAMA' : 'EXTRAIR EDITAL COM IA'}
              </DialogTitle>
            </DialogHeader>
          </div>
          ...
          <div className="p-6 space-y-6">
            {generatedSchedule ? (
              // Schedule preview
              <div className="space-y-4">
                <div className="p-4" style={{ border: '1px solid rgba(0,255,65,0.3)', background: '#020008' }}>
                  <div className="grid grid-cols-3 gap-3 mb-4">
                    <div>
                      <p className="font-pixel text-[9px] text-[#7f7f9f]">SEMANAS</p>
                      <p className="font-pixel text-xl text-[#00ff41]">{generatedSchedule.weeks}</p>
                    </div>
                    <div>
                      <p className="font-pixel text-[9px] text-[#7f7f9f]">TOTAL DE HORAS</p>
                      <p className="font-pixel text-xl text-[#00ff41]">{generatedSchedule.totalHours}h</p>
                    </div>
                    <div>
                      <p className="font-pixel text-[9px] text-[#7f7f9f]">TÓPICOS</p>
                      <p className="font-pixel text-xl text-[#00ff41]">{generatedSchedule.priorities?.length || 0}</p>
                    </div>
                  </div>
                </div>

                {generatedSchedule.keyMilestones && generatedSchedule.keyMilestones.length > 0 && (
                  <div className="p-3" style={{ border: '1px solid rgba(0,255,65,0.2)', background: '#020008' }}>
                    <p className="font-pixel text-[10px] text-[#00ff41] mb-2">📍 MARCOS IMPORTANTES</p>
                    <div className="space-y-1">
                      {generatedSchedule.keyMilestones.slice(0, 3).map((milestone, i) => (
                        <p key={i} className="font-mono text-xs text-[#7f7f9f]">• {milestone}</p>
                      ))}
                    </div>
                  </div>
                )}
                ...
                {generatedSchedule.tips && generatedSchedule.tips.length > 0 && (
                  <div className="p-3" style={{ border: '1px solid rgba(0,255,65,0.2)', background: '#020008' }}>
                    <p className="font-pixel text-[10px] text-[#00ff41] mb-2">💡 DICAS DE ESTUDO</p>
                    <div className="space-y-1">
                      {generatedSchedule.tips.slice(0, 3).map((tip, i) => (
                        <p key={i} className="font-mono text-xs text-[#7f7f9f]">• {tip}</p>
                      ))}
                    </div>
                  </div>
                )}
              ...
              <>
                ...
                <div className="grid gap-4">
                  <div className="grid gap-2">
                    <Label htmlFor="role" className="font-pixel text-[10px] text-[#00ff41]">
                      1️⃣ Cargo/Posição
                    </Label>
                    <Input
                      id="role"
                    ...
                    <p className="font-mono text-xs text-[#7f7f9f]">
                      Especifique o cargo para o qual as matérias serão extraídas
                    </p>
                  </div>
                  ...
                  <div className="grid gap-2">
                    <Label htmlFor="pdfFile" className="font-pixel text-[10px] text-[#00ff41]">
                      3️⃣ PDF do Edital
                    </Label>
                    <label
                      ...
                      {selectedFile ? (
                        <>
                          ...
                          <div className="text-center">
                            <p className="font-mono text-xs text-[#00ff41]">{selectedFile.name}</p>
                        ...
                        <>
                          ...
                          <span className="font-mono text-xs text-[#00ff41] text-center">
                            Clique para selecionar o PDF<br/>ou arraste um arquivo
                          </span>
                        </>
                      )}
                    </label>
                  ...
                  <div className="grid gap-2">
                    <Label htmlFor="examDate" className="font-pixel text-[10px] text-[#00ff41] flex items-center gap-2">
                  ...
                  <Button
                    ...
                    disabled={isParsing || !pageRanges.trim() || !selectedFile || !selectedRole.trim()}
                    className="w-full gap-2 font-pixel text-[10px] tracking-wider transition-all rounded-none"
                    style={{
                      backgroundColor: pageRanges.trim() && selectedFile && selectedRole.trim() ? '#00ff41' : '#7f7f9f',
                      color: '#04000a',
                      boxShadow: pageRanges.trim() && selectedFile && selectedRole.trim() ? '0 0 15px rgba(0,255,65,0.4)' : 'none',
                    }}
                  >
                    {isParsing ? (
                      <>
