...
export function LessonPanel({ lessonId, open, onOpenChange }: LessonPanelProps) {
        ...
        {loading ? (
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              ...
              <div className="font-mono text-base text-[#555]">Carregando lição...</div>
            </div>
          </div>
        ) : lesson ? (
          <>
            <SheetHeader className="mb-6">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  {lesson.description && (
                    <SheetDescription className="text-left mt-2">{lesson.description}</SheetDescription>
                  )}
                </div>
                <Badge variant={statusVariant} className="ml-4">{statusLabel}</Badge>
              </div>
            </SheetHeader>
                ...
                <div className="p-4 space-y-3">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="font-pixel text-[6px] text-[#555]">ESTIMADO</div>
                        <div className="font-mono text-base text-[#e0e0ff]">
                          {lesson.estimated ? `${lesson.estimated} min` : "Não definido"}
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="font-pixel text-[6px] text-[#555]">ESTUDADO</div>
                        <div className="font-mono text-base text-[#e0e0ff]">
                  ...
                  {lesson.externalUrl && (
                    <>
                      <Separator />
                      <div className="font-pixel text-[6px] text-[#555] mb-2">LINK EXTERNO</div>
                      ...
                      <div className="space-y-2 max-h-48 overflow-y-auto">
                        {lesson.studyLogs.map((log: { id: Key | null | undefined; createdAt: string | number | Date; minutes: number }) => (
                          <div key={log.id}
                            ...
                            style={{ border: '1px solid rgba(0,255,65,0.15)', background: '#020008' }}>
                            <div className="flex items-center gap-2">
