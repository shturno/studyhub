...
export function MaterialManager({ topicId }: { readonly topicId: string }) {
  ...
  const loadMaterials = useCallback(async () => {
    const data = await getMaterials(topicId).catch(() => [] as Material[]);
    setMaterials(data);
    setIsLoading(false);
  }, [topicId]);
  ...
  return (
    <div className="space-y-3">
      ...
      {isLoading && (
        <div className="font-mono text-sm text-[#555] text-center py-4">
          Carregando...
        </div>
      )}
      {!isLoading && materials.length === 0 && (
        <div
          className="font-mono text-sm text-[#444] text-center py-6"
