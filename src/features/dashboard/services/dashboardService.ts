...
import { getWeeklyTip } from "@/features/insights/services/weeklyTipService";
import { getOrCreateTodayObligation, applyPendingPenalties } from "@/features/gamification/services/dailyObligationService";
import { getActivityFeed } from "@/features/gamification/services/activityEventService";
...
export async function getDashboardData(
  ...
  const [
    ...
    // Today's planned sessions (for planned-vs-actual widget)
    prisma.plannedSession.findMany({
      where: {
        userId,
        scheduledDate: { gte: startOfDay(new Date()), lte: endOfDay(new Date()) },
      },
  ...
  const weeklyTip = await getWeeklyTip(userId).catch(() => null);

  // Apply pending XP penalties (lazy — no cron needed)
  const pendingPenalties = await applyPendingPenalties(userId).catch(() => ({
    totalPenalty: 0,
    missedDays: [],
  }));

  const difficultSubjectsRaw = Array.isArray(userSettings.difficultSubjects)
    ? (userSettings.difficultSubjects as (DifficultSubject | string)[])
    : [];
  ...
  if (difficultSubjectIds.length > 0) {
    const today = format(new Date(), "yyyy-MM-dd");
    const todayObligation = await prisma.dailyObligation.findUnique({
      where: { userId_date: { userId, date: today } },
      include: { topic: { include: { subject: true } } },
    });

    if (todayObligation && !difficultSubjectIds.includes(todayObligation.topic.subject.id)) {
      await prisma.dailyObligation.delete({
        where: { id: todayObligation.id },
      });
    }
  }

  // Daily obligation (Gemini-powered) + activity feed
  const [dailyObligation, activityFeed] = await Promise.all([
    getOrCreateTodayObligation(userId, contestWithTopics?.id).catch(() => null),
    getActivityFeed(userId, 20).catch(() => []),
  ]);

  // Today's planned sessions enriched
  const todayPlanned = todayPlannedRaw.map((p) => ({
    id: p.id,
    topicId: p.topicId,
  ...
  return {
    ...
    contestId: contestWithTopics?.id ?? null,
    priorities,
    heatmap,
