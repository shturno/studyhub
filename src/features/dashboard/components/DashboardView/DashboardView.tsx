...
import { Trophy, BookOpen, Clock, ArrowUpRight, Flame } from "lucide-react";
...
import { PenaltyModal } from "@/features/gamification/components/PenaltyModal/PenaltyModal";
import { DailyObligationCard } from "../DailyObligationCard/DailyObligationCard";
import { InlineTimer } from "../InlineTimer/InlineTimer";
import { ActivityFeed } from "../ActivityFeed/ActivityFeed";
import { PlannedVsActualCard } from "../PlannedVsActualCard/PlannedVsActualCard";

export function DashboardView({ data, contests = [], aiSlot }: DashboardViewProps) {
  const { user, nextTopic, recentSessions, heatmap, statsData, streak, xpProgress, xpToNextLevel, dailyGoal, weeklyComparison, missions, dailyObligation, activityFeed, todayPlanned, pendingPenalties } = data;

  const [penaltyDismissed, setPenaltyDismissed] = useState(false);
  const [timerExpanded, setTimerExpanded] = useState(false);

  const handleStudyNow = useCallback(() => {
    setPenaltyDismissed(true);
    setTimerExpanded(true);
  }, []);

  const hasPendingPenalties = !penaltyDismissed && (pendingPenalties?.totalPenalty ?? 0) > 0;
  ...
  return (
    <div className="min-h-screen bg-[#080010] p-4 md:p-6 space-y-6">
      {/* Blocking Penalty Modal */}
      {hasPendingPenalties && pendingPenalties && (
        <PenaltyModal
          pendingPenalties={pendingPenalties}
          onDismiss={() => setPenaltyDismissed(true)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          {/* Daily Obligation Card */}
          {dailyObligation && (
            <DailyObligationCard
              obligation={dailyObligation}
              onStudyNow={() => setTimerExpanded(true)}
            />
          )}

          {/* Inline Timer — zero navigation study */}
          <InlineTimer
            dailyObligation={dailyObligation}
            nextTopic={nextTopic}
            forceExpanded={timerExpanded}
            onExpandedChange={setTimerExpanded}
          />

          {/* Planned vs Actual */}
          {todayPlanned && todayPlanned.length > 0 && (
            <PlannedVsActualCard planned={todayPlanned} />
          )}
        </div>
      ...
      {/* Activity Feed — replaces recentSessions (shows gains + losses) */}
