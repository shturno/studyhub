...
const BLOCKS = Array.from({ length: 20 }, (_, n) => ({ id: `pb-${n}`, index: n }))
...
export function TimerDisplay({ topicId, topicName, subjectName, onComplete }: TimerDisplayProps) {
    ...
    return (
        ...
        <div className="text-center space-y-2">
                <div className="font-pixel text-[8px] text-[#7f7f9f] tracking-widest">{subjectName.toUpperCase()}</div>
                <div className="font-mono text-3xl text-[#e0e0ff] leading-tight">{topicName}</div>
            </div>

            <div className="relative w-full" style={{ border: '4px solid #00ff41', background: '#020008', padding: '32px 24px' }}>
                ...
                <div className="text-center relative z-10">
                    <div className={cn(
                        ...
                        style={isRunning ? { textShadow: '0 0 20px rgba(0,255,65,0.8), 0 0 40px rgba(0,255,65,0.4)' } : {}}>
                        {formattedTime}
                    </div>
            ...
            <div className="w-full space-y-1">
                <div className="font-pixel text-[7px] text-[#7f7f9f] text-right">{Math.round(progress)}%</div>
                <div className="w-full h-6 flex gap-1" style={{ border: '2px solid #00ff41', padding: '3px', background: '#020008' }}>
                    {BLOCKS.map((block) => (
                        <div key={block.id} className="flex-1 h-full transition-all duration-300"
                            style={{
                                background: block.index < filledBlocks ? '#00ff41' : '#0d001a',
                                boxShadow: block.index < filledBlocks ? '0 0 6px rgba(0,255,65,0.6)' : 'none',
                            }} />
                    ))}
                </div>
            </div>
            ...
            <div className="flex gap-2">
                {durations.map((d) => (
                    <button key={d}
                        ...
                        style={{ border: isRunning ? '2px solid #333' : '2px solid #00ff41' }}>
                        {d}m
                    </button>
                ))}
            </div>
            ...
            <div className="flex items-center gap-4">
                ...
                {isRunning ? (
                    <button onClick={pause}
                        ...
                        style={{ background: '#0d001a', border: '2px solid #e0e0ff', boxShadow: '4px 4px 0px #333' }}>
            ...
            <div className="h-16 flex items-center justify-center">
                {isSaving && (
                    <div className="font-pixel text-[8px] text-[#ffbe0b] animate-blink"
                ...
                {hasCompleted && !isSaving && (
                    <div className="text-center space-y-3">
                        <div className="font-pixel text-[10px] text-[#00ff41] flex items-center gap-2"
