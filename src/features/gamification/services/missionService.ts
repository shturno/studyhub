...
export async function getOrCreateTodayMissions(
  ...
  // — CORE MISSION: sempre STUDY_MINUTES 15min como primeira missão obrigatória —
  const hasCore = existing.some((m) => m.isCore);
  if (!hasCore) {
    await prisma.dailyMission.upsert({
      where: { userId_date_type: { userId, date, type: "STUDY_MINUTES" } },
      create: {
        userId,
        date,
        type: "STUDY_MINUTES",
        label: "Missão Obrigatória: estude 15 minutos hoje",
        targetValue: 15,
        xpReward: 30,
        isCore: true,
      },
      update: {
        isCore: true, // marca como core se já existia
      },
    });
  }

  // Re-fetch after upsert
  const afterCore = await prisma.dailyMission.findMany({
    where: { userId, date, NOT: { type: BONUS_SENTINEL_TYPE } },
    orderBy: { createdAt: "asc" },
  });

  const usedTypes = new Set(afterCore.map((m) => m.type));
  const needed = MISSIONS_PER_DAY - afterCore.length;
  ...
  const all = await prisma.dailyMission.findMany({
    where: { userId, date, NOT: { type: BONUS_SENTINEL_TYPE } },
    orderBy: [{ isCore: "desc" }, { createdAt: "asc" }], // core sempre primeiro
  });
