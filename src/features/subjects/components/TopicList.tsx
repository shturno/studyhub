...
import { Play, CheckCircle2, Circle, ChevronRight, ChevronDown, Plus } from 'lucide-react'
...
function TopicItem({ node, level }: { readonly node: TopicNode; readonly level: number }) {
            ...
            >
                <div className="flex items-center gap-3">
                    <button
                        ...
                        className={cn("p-1 text-[#555] hover:text-[#00ff41] transition-colors", !hasChildren && "invisible")}
                    >
                        {isOpen ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
                    </button>

                    {node.status === 'mastered' && <CheckCircle2 className="w-4 h-4" style={{ color: '#00ff41' }} />}
                    {node.status === 'studied' && <CheckCircle2 className="w-4 h-4" style={{ color: '#7b61ff' }} />}
                ...
                <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
                        <DialogTrigger asChild>
                            <Button size="icon" variant="ghost" className="h-7 w-7" title="Adicionar Sub-tópico">
