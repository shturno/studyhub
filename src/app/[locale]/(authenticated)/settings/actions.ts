...
export async function updateDifficultSubjects(
  payload: Array<{ contestId: string; subjectId: string }>,
): Promise<ActionResult<void>> {
  try {
    const session = await auth();
    if (!session?.user?.id) {
    ...
    console.error("updateDifficultSubjects error:", error);
    if (error instanceof Error) {
      return err(error.message);
    }
    return err("Erro ao atualizar matérias difíceis");
  }
}
