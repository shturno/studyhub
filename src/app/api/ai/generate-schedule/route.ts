...
import { getLearningContext } from "@/features/insights/services/learningContextService";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const schedule = await generateScheduleWithGemini({
      ...
      focusAreas,
      learningContext: await getLearningContext(session.user.id)
        .then(({ promptText }) => promptText || undefined)
        .catch(() => undefined),
    });
